...
class train_dataset(torch.utils.data.Dataset):
    def __init__(self, mode='train', root_path=None,
                 ...
                 is_rrot=False, rand_hori_flip=True, rand_vert_flip=True, rgb_shuffle=True, grayscale=False):
    ...
    def __getitem__(self, index):
        ...
        img_lr = np.ascontiguousarray(np.transpose(np.array(img_lr), (2, 0, 1)))
...
class test_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_lr = np.ascontiguousarray(np.transpose(np.array(img_lr), (2, 0, 1)))
        img_hr = np.ascontiguousarray(np.transpose(np.array(img_hr), (2, 0, 1)))
        img_lr = torch.from_numpy(img_lr).float()
